<?php
class fproduct extends MY_Controller{
    ...
    public function newProducts(){
        ...
        $newproducts->order_by('created', 'asc');
        $newproducts->where('isNew', 1);
        $newproducts->get()->all;
        $dis['newproducts'] = $newproducts;
        
        $this->page_title = "Sản phẩm mới";
        
        $dis['base_url']=base_url();
        $dis['link'] = gen_seo_url($this->lang->line('news')).'/';
        $dis['view']='front/product/product-new';
        $this->viewfront($dis) ;
    }
    
    public function uniqueProducts(){
        $uniqueProduct = new Product();
        $uniqueProduct->order_by('created', 'asc');
        $uniqueProduct->where('isGift', 1);
        $uniqueProduct->get()->all;
        $dis['uniqueProduct'] = $uniqueProduct;
        
        $this->page_title = "Sản phẩm độc đáo";
        
        $dis['base_url']=base_url();
        $dis['link'] = gen_seo_url($this->lang->line('news')).'/';
        $dis['view']='front/product/product-unique';
        $this->viewfront($dis) ;
    }
    
    public function hotProducts(){
        $hotproducts = new Product();
        $hotproducts->order_by('created', 'asc');
        $hotproducts->where('isHot', 1);
        $hotproducts->get(3);
        $dis['hotproducts'] = $hotproducts;
        
        $this->page_title = "Sản phẩm nổi bật";
        
        $dis['base_url']=base_url();
        $dis['link'] = gen_seo_url($this->lang->line('news')).'/';
        $dis['view']='front/product/product-hot';
        $this->viewfront($dis) ;
    }
    ...
    public function listByCat($url, $offset=0, $limit=10){
		...
		if($this->lang->lang() == "vi"){
            ...
            $this->page_title = $productCat->seo_title_vietnamese ;
            $this->page_description = $productCat->seo_description_vietnamese.' - Trang '.$this->getPageNumber($page);
            $this->page_keyword = $productCat->seo_keyword_vietnamese;
		}
        if($this->lang->lang() == "en"){
            ...
            $this->page_title = $productCat->seo_title_english ;
            $this->page_description = $productCat->seo_description_english.' - Trang '.$this->getPageNumber($page);
            $this->page_keyword = $productCat->seo_keyword_english;
		}

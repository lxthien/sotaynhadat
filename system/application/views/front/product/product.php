<div class="col-right fl">
    <div class="groupcol2">
        <div class="constructioncol2">
            <div class="titleconstructioncol2"><h1><?=lang('products')?></h1></div>
            <span class="line730"></span>
            <div class="list-product">
                <?php $i=0; foreach($product as $row): $i++;
                    $productCat = new Productcat($row->productcat_id);
                    ?>
                    <div class="list-product-item <?php if($i%4==0) echo 'list-product-item-break'; ?>">
                        <div class="img-list-product-item">
                            <a href="<?= $this->lang->lang() == 'vi' ? $base_url.'vi/'.$productCat->url_vietnamese.'/'.$row->url_vietnamese.'.html' : $base_url.'en/'.$productCat->url_english.'/'.$row->url_english.'.html'; ?>" title="<?=$row->{'name_'.$this->language}?>">
                                <img src="<?= $base_url.$row->image ?>" width="171" height="131" alt="<?=$row->{'name_'.$this->language}?>"/>
                            </a>
                        </div>

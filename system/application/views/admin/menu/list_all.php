...
<form name="frmMenu" id="frmMenu" method="post" >
   <?php $seg=$this->uri->segment(4,0);?>
	<table class="datatable" width="100%" cellpadding="0" cellspacing="0" id="box-table-a" summary="Employee Pay Sheet" >
        ...
        <?php $i=$this->uri->segment(4,0); foreach($object->all as $row):$i++;?>
        <tr>
            <td widtd="6%"><div align="center"><?php echo $i;?></div></td>
            <td><div align="left"><a href="<?=$this->admin_url.'menuitems/list_by_cat/'.$row->id?>"><?=$row->name?></a></div></td>
            ...
            <td>
                <div align="center">
                    <?php
                        ...
                        echo create_link_table('edit_icon',$this->admin_url.'menus/edit/'.$row->id,'Edit News');
    				   // echo create_link_table('delete_icon',$this->admin_url.'menus/delete/'.$row->id,'delete','return confirm ("Bạn có muốn xóa đối tượng này không ?")');
                    ?>
                </div>
            </td>
            <td widtd="33%">

...
<form id="frmProduct" class="table_input" action="<?=$base_url;?>admin/jobscats/edit/<?=$object->id;?>" method="post" enctype="multipart/form-data">
...
<table class="table_input">
    ...
    <tr>
     	<td>
			<label for="name">Tên danh mục (<?=$row->name?>):<span style="color: red;">*</span></label></td>
    ...
    <tr>
        ...
        <td><input  name="isGeneratorUrl" type="checkbox" checked="checked" value="1" /></td>
    </tr>
    <tr>
     	<td>
			<label for="name">URL:</label></td>
        <td><input type="text" id="url"   class="smallInput big"  name="url" value="<?=$object->url;?>" /></td>
    </tr>
    <?php foreach($sitelanguage as $row): ?>
    <tr>
        <td><label for="name">SEO - Description (<?=$row->name?>):</label></td>
        <td><textarea  name="description_<?=$row->short?>"  class="smallInput big"><?=$object->{'description_'.$row->short};?></textarea></td>
    </tr>
    <?php endforeach ?>
    <?php foreach($sitelanguage as $row): ?>
    <tr>
        <td><label for="name">SEO - Keyword (<?=$row->name?>):</label></td>
        <td><textarea  name="keyword_<?=$row->short?>"  class="smallInput big"><?=$object->{'keyword_'.$row->short};?></textarea></td>
    </tr>
    <?php endforeach ?>
</table>
<!--end content tabs-->
